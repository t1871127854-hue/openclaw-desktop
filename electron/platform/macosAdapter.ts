...
export class MacOSAdapter extends BasePlatformAdapter {
  ...
  async planInstall(context: PlatformAdapterContext): Promise<InstallPlan> {
    ...
    const plan: InstallPlan = {
      ...
      steps: [
        ...
        {id: 'install-runtime', title: 'Install native runtime', status: compatibility.level === 'unsupported' ? 'blocked' : 'ready'},
      ],
    };
  ...
  async installRuntime(plan: InstallPlan): Promise<PlatformOperationResult> {
    ...
    executedSteps.push(await this.installNodeArchive(nodeResource));
    executedSteps.push(await this.extractArchiveResource(runtimeResource, path.join(this.options.paths.runtimeRoot, 'runtime'), 'deploy-runtime'));
    executedSteps.push(await this.extractArchiveResource(gatewayResource, path.join(this.options.paths.runtimeRoot, 'gateway'), 'deploy-gateway'));

    const validation = await this.validateMacInstall(prepared.resolved.resources);
  ...
  private async validateMacInstall(resources: ResourceDefinition[]): Promise<PostInstallValidationDetails> {
