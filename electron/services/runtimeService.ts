...
export class RuntimeService {
  ...
  private buildFailureInfo(
    ...
    const offlineMissingFiles = Array.isArray(details?.offlineMissingFiles)
      ? details.offlineMissingFiles.map((item) => String(item))
      : [];
    ...
    if ((stage === 'import-download-resources' || stage === 'install-runtime') && offlineMissingFiles.length > 0) {
      return {
        stage,
        reason: normalizedReason,
        userFacingMessage: `离线资源不完整，当前无法继续${stage === 'install-runtime' ? '安装运行时' : '准备安装资源'}。`,
        technicalDetails,
        suggestedActions: [
          `补齐缺失资源：${offlineMissingFiles.join('、')}。`,
          '若暂时无法补齐，请切换到 online 或 hybrid 模式。',
          '补齐 manifest.json、rootfs / 镜像包、校验文件后再重试。',
        ],
        retryable: true,
    ...
    if (stage === 'install-runtime' && /not implemented yet|stub/.test(lowerReason)) {
      return {
        stage,
        reason: normalizedReason,
        userFacingMessage: '当前平台的运行时安装步骤仍是 Beta 骨架，尚未接入真实安装执行。',
        technicalDetails,
        suggestedActions: [
          '当前版本可继续验证状态检测、资源准备、失败摘要与日志链路。',
          '如要完成真实安装，请先确认目标平台 adapter 是否已实现 installRuntime。',
          '如果只是离线资源不足，请先补齐 offline_resources，或改用 online / hybrid 模式验证资源链。',
        ],
        retryable: false,
        recommendedModeSwitch: 'hybrid',
        reportExportable: true,
  ...
  private async setInstallFailure(stage: string, reason: string, details?: Record<string, unknown>) {
    ...
    if (stage === 'import-download-resources' || stage === 'install-runtime') {
      const offlineResources = await this.detectOfflineResources().catch(() => null);
      if (offlineResources) {
        enrichedDetails = {
          ...(details ?? {}),
          offlineMissingFiles: offlineResources.missingFiles,
          offlineModeSuggestion: offlineResources.modeSuggestion,
          offlineAdvice: offlineResources.advice.slice(0, 3),
        };
      }
    }

    const failure = this.buildFailureInfo('install', stage, reason, enrichedDetails);
